...
namespace ZPBot.SilkroadSecurityApi
{
	public class Blowfish
	{
        ...
        private uint bf_F(uint x) => ((S(x, 0) + S(x, 1)) ^ S(x, 2)) + S(x, 3);

	    private void Round(ref uint a, uint b, int n) => a ^= bf_F(b) ^ _pArray[n];
		...
		public int GetOutputLength(int length) => length % 8 == 0 ? length : length + (8 - length % 8);
